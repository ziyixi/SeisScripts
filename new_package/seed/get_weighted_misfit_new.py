...
def main(body_json, surf_json, bin_angle):
    ...
    # take p_z as an example, inspect misfit
    bbq = 0
    for index, item_win in enumerate(p_z):
        print(index, item_win, item_win.misfit/item_win.length)
        bbq += item_win.misfit*item_win.azimuth_weight
        if(item_win.misfit > 0.01 or item_win.azimuth_weight > 0.01):
            print("!!!!!")
    print(bbq)
    # some windows may have problem
    # misfit_windows(channel='z', phase='p', misfit=3998410.0096028517, length=80.0, amplitude=326046.16812033, gcarc=35.32402516032001, azimuth=269.52446451584115, azimuth_weight=0.0003184532386176048)
    # misfit_windows(channel='z', phase='p', misfit=0.7669534689549725, length=80.0, amplitude=0.07212283525606346, gcarc=33.531410836060545, azimuth=295.3536515298188, azimuth_weight=0.00032469741976696954)
